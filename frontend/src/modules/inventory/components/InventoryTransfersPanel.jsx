...
/**
 * INV-002 — request → approve → dispatch → in transit → receive, both branches
 * reconcile. Body of the former InventoryTransfersPage; every PermissionGuard
 * from that page is carried over unchanged.
 */
export function InventoryTransfersPanel() {
  ...
  const branchName = (id) =>
    branches.find((b) => b.id === id)?.displayName || branches.find((b) => b.id === id)?.name || '—';
    ...
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">
        {t(
          ...
          'Request → approve → dispatch → in transit → receive. Both branches reconcile on receipt.'
        )}
      </p>

      <PermissionGuard
        permissions={[PERMISSIONS.INVENTORY_TRANSFER_REQUEST, PERMISSIONS.INVENTORY_ALL]}
      >
        <Card>
          <CardHeader>
            <CardTitle>{t('inventory.transfers.requestATransfer', 'Request a transfer')}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={onSubmit} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, fromBranchId: e.target.value, fromItemId: '' }))
                  }
                >
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('inventory.transfers.transfers', 'Transfers')}</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              ...
              {!isLoading && transfers.length === 0 && (
                <TableRow>
                  <TableCell colSpan={5} className="text-center text-muted-foreground">
                    {t('inventory.transfers.noTransfers', 'No transfers yet.')}
                  </TableCell>
                </TableRow>
              )}
              {transfers.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell className="flex items-center gap-1 text-sm">
                    {branchName(row.fromBranchId)} <ArrowRight className="h-3 w-3" />{' '}
                    {branchName(row.toBranchId)}
                  </TableCell>
                  <TableCell>{row.quantityRequested}</TableCell>
                  <TableCell>
                    ...
                    {row.status === 'REQUESTED' && (
                      ...
                      >
                        <Button size="sm" variant="outline" onClick={() => approve.mutate(row.id)}>
                          {t('inventory.transfers.approve', 'Approve')}
                        </Button>
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() =>
                            reject.mutate({ id: row.id, reason: 'Rejected by staff' })
                          }
                        >
                          <PackageX className="h-3.5 w-3.5" />{' '}
                          {t('inventory.transfers.reject', 'Reject')}
                        </Button>
                      </PermissionGuard>
                    )}
                    {row.status === 'APPROVED' && (
                      ...
                      >
                        <Button
                          ...
                          onClick={() =>
                            dispatch.mutate({
                              id: row.id,
                              payload: { quantityDispatched: row.quantityRequested },
                            })
                          }
                        >
                          <Truck className="h-3.5 w-3.5" />{' '}
                          {t('inventory.transfers.dispatch', 'Dispatch')}
                        </Button>
                      </PermissionGuard>
                    )}
                      ...
                      >
                        <Button
                          ...
                          onClick={() =>
                            receive.mutate({
                              id: row.id,
                              payload: { quantityReceived: row.quantityDispatched },
                            })
                          }
                        >
                          <PackageCheck className="h-3.5 w-3.5" />{' '}
                          {t('inventory.transfers.receive', 'Receive')}
                        </Button>
                      </PermissionGuard>
                    )}
                    {row.status === 'RECEIVED' && row.varianceQuantity !== 0 && (
                      <span className="text-xs text-warning">
                        {t('inventory.transfers.variance', 'Variance')}: {row.varianceQuantity}
                      </span>
                    )}
                  </TableCell>
                </TableRow>
...
export default InventoryTransfersPanel;
