...
/**
 * The treatment-plan wizard body (protocol → package → consents → approve), extracted so it can be
 * mounted both by the standalone builder page (`TreatmentPlanBuilderPage`) AND inline inside the
 * Consultation Workspace's "Treatment" tab (§3.5). All hooks/mutations live here so there is exactly
 * one implementation of plan-editing logic.
 *
 * `embedded` trims the outer card border/max-width so the caller controls layout; the page still
 * owns the header, print and invoice links.
 */
export function TreatmentPlanForm({ planId, embedded = false }) {
  const { t } = useTranslation();
  const { data: plan, isLoading, isError, error } = useTreatmentPlan(planId);
  const update = useUpdateTreatmentPlan(planId);
  const applyProtocol = useApplyProtocol(planId);
  const applyPackage = useApplyPackage(planId);
  const recommend = useRecommendPlan(planId);
  ...
  const acceptConsent = useAcceptConsent(planId);
    ...
    <div className={cn('space-y-4', embedded ? '' : 'space-y-6')}>
      ...
      {step === 1 && (
        <div className="space-y-3 rounded-xl border p-4">
          ...
          {!readOnly && (
            <PermissionGuard permissions={[PERMISSIONS.TREATMENT_PLAN_EDIT, PERMISSIONS.TREATMENT_PLAN_ALL]}>
