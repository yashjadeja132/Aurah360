    ...
    <PermissionGuard permissions={[PERMISSIONS.LOYALTY_CAMPAIGNS_MANAGE, PERMISSIONS.LOYALTY_ALL]} fallback="redirect">
      <section className="space-y-6">
        ...
        <div className="space-y-2">
          {campaigns.map((c) => (
            <div key={c.id} className="flex flex-col gap-2 rounded-xl border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
          ...
          {!campaigns.length && !isLoading && (
            <p className="text-sm text-muted-foreground">{t('loyalty.campaigns.empty', 'No campaigns yet.')}</p>
          )}
        </div>
